...
OpenVLA 공식 LIBERO-Spatial fine-tuned checkpoint를 사용해 Task 0–9를 각 50회씩 평가한 결과를 GitHub에서 바로 읽을 수 있도록 정리한 문서 묶음이다.
...
1. [docs/benchmark_report.md](docs/benchmark_report.md) — 그래프 중심의 최종 요약
2. [docs/failure_analysis.md](docs/failure_analysis.md) — Task 4를 포함한 실패 원인 분석
3. [docs/architecture.md](docs/architecture.md) — LIBERO와 OpenVLA의 Mermaid 아키텍처
...
OpenVLA 원본 저장소에 넣는다면 다음처럼 benchmark 전용 subtree로 두는 구성이 가장 관리하기 쉽다.

~~~text
benchmarks/libero_spatial_openvla/
~~~

이 묶음 자체는 이미 그 subtree에 복사할 수 있도록 구성되어 있다. 실제 경로와 복사 명령은 docs/file_map.md에 있다.

## 범위와 제외 사항

이 문서는 **LIBERO benchmark 측정만** 다룬다. Isaac Sim, 실제 Franka, scene graph, recovery policy, 추가 controller, Grounding DINO/SAM2는 이번 benchmark 점수에 섞지 않는다. 그런 요소를 추가하면 공식 OpenVLA baseline과 비교할 때 별도 실험 조건이 된다.
...
원본 비디오와 NPZ는 media/task_04/에 Task 4 사례로만 포함했다. 전체 500 episode의 영상은 저장소 크기와 검증 비용을 크게 늘리므로, 필요하면 Git LFS 또는 GitHub Release/Zenodo에 별도로 올리는 것을 권장한다.
